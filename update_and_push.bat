...
:: 1. Run Check API Status
echo [1/3] Checking SmartFix API Status...
python check_api_status.py
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo [ERROR] check_api_status.py failed!
    echo Aborting update to protect data.
    pause
    exit /b %ERRORLEVEL%
)

:: 2. Run Station Health (DISABLED per user request to avoid connection limits)
:: echo.
